...
class AdminController {
  ...
  async login(params: AdminRequest.Login) {
    try {
      ...
      if (!step1 || (step1?.userType !== USER_TYPE.ADMIN && step1?.userType !== USER_TYPE.SUB_ADMIN))
      ...
      if (!isPasswordMatched)
        ...
        let authToken = await axiosService.postData({
          url: SERVER.AUTH_APP_URL + SERVER.CREATE_AUTH_TOKEN,
          body: tokenData,
        });
  ...
  async forgotPassword(params: AdminRequest.ForgotPasswordRequest) {
    try {
      ...
      if (otp_count && JSON.parse(otp_count).count >= 3)
      ...
      mailData = {
        ...
        email: params.email,
        name: step1.name,
        link: `${SERVER.ADMIN_URL}/account/reset-password?token=${authToken.data.jwtToken}`,
      };
  ...
  async adminDetails(tokenData: TokenData) {
    try {
      ...
      let details = {
        ...
        allnotificationsSeen: admin?.allnotificationsSeen
      };
      if (admin.userType === USER_TYPE.SUB_ADMIN) {
        const rolesData = await adminDaoV1.getRoleDetailsById(admin.roleId);
        details['permission'] = rolesData.permissions
      }
  ...
  async editProfile(params: AdminRequest.EditProfile, tokenData: TokenData) {
    try {
      ...
      if (params.deviceToken) {
        await adminDaoV1.updateDeviceToken(tokenData.userId, params.deviceToken);
      }
