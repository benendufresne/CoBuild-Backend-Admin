...
import { addFaq, contentDetails, editContent, editFaq, faq, faqDetails, faqList, getContent, viewContent } from "./routeValidate";
...
export const contentRoute = [
    ...
    handler: async (request: Request | any, h: ResponseToolkit) => {
      try {
        ...
        if (tokenData.userType != USER_TYPE.USER)
          await subAdminControllerV1.hasPermissionAccess(tokenData.userId, PERMISSION.CMS_MANAGEMENT, PERMISSION_TYPE.VIEW)
  ...
  },
  {
    ...
    config: {
      ...
      description: "Content Details",
      auth: {
        strategies: ["BasicAuth", "CommonAuth"],
      },
      validate: {
  ...
  },
  {
    ...
    handler: async (request: Request | any, h: ResponseToolkit) => {
      try {
        ...
        if (tokenData.userType != USER_TYPE.USER)
          await subAdminControllerV1.hasPermissionAccess(tokenData.userId, PERMISSION.CMS_MANAGEMENT, PERMISSION_TYPE.EDIT)
