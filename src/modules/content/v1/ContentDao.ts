...
export class ContentDao extends BaseDao {
	...
	async faqList(params: ListingRequest) {
		try {
			...
			// }
			// else{
			project = {id: 1, question: 1, answer: 1, created: 1, position: 1, status: 1}
			// }
			aggPipe.push({ "$project": project });

			aggPipe.push({ "$sort": { position: 1 } });

			const response = await this.dataPaginate(this.modelContent, aggPipe, params.limit, params.pageNo, {}, true);
			return response;
		} catch (error) {
			console.log("Error", error)
			throw error;
