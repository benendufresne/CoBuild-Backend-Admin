...
export class ContentController {
	...
	async addFaq(params: ContentRequest.AddFaq) {
		try {
			...
			const faqCount = await baseDao.countDocuments(this.modelContent, { type: CONTENT_TYPE.FAQ });
			const faqPosition = await baseDao.findOne(this.modelContent, { "type": CONTENT_TYPE.FAQ, "position": params.position }, { _id: 1 });
